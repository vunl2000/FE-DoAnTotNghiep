export const dataAll = [
  {
    id: '1',
    headerTitle: 'Vấn đề đơn hàng',
    nameIcon: 'receipt',
    data: [
      {
        id: 1,
        title: 'Tôi có thể thay đổi hoặc sửa đổi đơn đặt hàng của mình không?',
        titlebody:
          'Xin chào, chúng tôi có thể giúp bạn sửa đổi đơn đặt hàng trước khi vận chuyển, ví dụ như thực hiện hủy, trao đổi các mặt hàng hoặc địa chỉ giao hàng và hoàn lại tiền, nhưng không bao gồm thêm các mặt hàng. Bạn có thể liên hệ với chúng tôi để được trợ giúp thêm.',
        checkItem: false,
      },
      {
        id: 2,
        title:
          'Tại sao tôi không nhận được email xác nhận về đơn hàng của mình?',
        titlebody:
          'Xin chào. Trong trường hợp thông thường, bạn sẽ nhận được email xác nhận trong vòng 24 giờ sau khi thanh toán. Đôi khi, email xác nhận có thể được phân loại thư rác của bạn. Nếu bạn không thể tìm thấy nó trong tài khoản email của mình, vui lòng đăng nhập trực tiếp vào tài khoản Fashion của bạn và nhấp vào" Đơn hàng của tôi" để theo dõi trực tiếp trạng thái vận chuyển bưu kiện của bạn. Hoặc bạn cũng có thể liên hệ bằng Ticket của tôi. Sau đó, chúng tôi sẽ cung cấp các chi tiết của gói hàng của bạn cho bạn ngay lập tức.Chúc một ngày tốt lành~',
        checkItem: false,
      },
      {
        id: 3,
        title: 'Mất bao lâu đơn hàng của tôi sẽ được vận chuyển?',
        titlebody:
          'Thông thường, sẽ mất 1-3 ngày để chuẩn bị các mặt hàng. \n (Đôi khi nó sẽ bị trì hoãn do vấn đề hàng tồn kho)\nDo ảnh hưởng của dịch COVID-19 hiện nay, việc thiếu tài nguyên vận tải toàn cầu có thể làm trì hoãn đơn đặt hàng của bạn. Trong quá trình xử lý, đơn hàng của bạn có thể bị chậm từ 4-6 ngày, và tùy thuộc vào công ty vận chuyển, xin vui lòng chờ đợi.Khi đã sẵn sàng, chúng tôi sẽ gửi đơn hàng của bạn. Chúng tôi sẽ gửi email cho bạn về số theo dõi và liên kết theo dõi khi nó được chuyển đi.',
        checkItem: false,
      },
      {
        id: 4,
        title: 'Làm cách nào để kiểm tra trạng thái đơn đặt hàng của tôi',
        titlebody:
          'Thông thường, bạn có thể kiểm tra trạng thái đơn hàng của mình thông qua "Đơn hàng của tôi"\n Ngoài ra, bạn cũng có thể liên hệ với chúng tôi để kiểm tra trực tiếp.',
        checkItem: false,
      },
      {
        id: 5,
        title:
          'Tại sao tôi không nhận được email về đơn hàng của mình được chuyển đi?',
        titlebody:
          'Thông thường, một email với số theo dõi sẽ được gửi đến địa chỉ email tài khoản của bạn sau khi đơn hàng được vận chuyển trong 24 giờ.\n Nếu bạn không nhận được email:\n 1.Vui lòng truy cập "Đơn hàng của tôi" để kiểm tra trạng thái đơn hàng, nó có thể vẫn đang được xử lý.\n2.Vui lòng kiểm tra xem bạn đã đăng nhập đúng tài khoản Fashion chưa, email được gửi đến địa chỉ email đã đăng ký chưa.\n 3. Vui lòng kiểm tra các email rác của bạn và địa chỉ email của bạn có che chắn email của chúng tôi không .',
        checkItem: false,
      },
      {
        id: 6,
        title: 'Làm cách nào để thay đổi hoặc sửa đổi địa chỉ đã thanh toán?',
        titlebody:
          '"Trên hệ thống của chúng tôi, địa chỉ thanh toán sẽ được tự động tạo ra giống như địa chỉ giao hàng, nếu bạn muốn thay đổi địa chỉ đó, bạn có thể nhấp vào chỉnh sửa tại trang thanh toán (điều này sẽ chỉ được hoạt động khi thanh toán lần đầu).Nếu bạn đã thanh toán, thì không cần thay đổi địa chỉ thanh toán, vì chúng tôi sẽ gửi đơn đặt hàng đến địa chỉ giao hàng mà bạn đã để lại trên trang web của chúng tôi.Do đó, không có vấn đề gì nếu địa chỉ thanh toán khác với địa chỉ giao hàng. "',
        checkItem: false,
      },
      {
        id: 7,
        title: 'Tôi có thể hủy đơn hàng không?',
        titlebody:
          '"Thông thường, các đơn đặt hàng chưa thanh toán hoặc chưa được chuyển có thể hủy qua "Đơn hàng của tôi".\nNhưng xin lưu ý rằng một khi đơn hàng đã được chuyển đi, thì không được phép hủy nữa. Hy vọng bạn có thể thông cảm.\nChúc một ngày tốt đẹp."',
        checkItem: false,
      },
    ],
    check: false,
  },
  {
    id: '2',
    headerTitle: 'Vận chuyển',
    nameIcon: 'bus',
    data: [
      {
        id: 1,
        title:
          'Tôi có thể nhận được đơn đặt hàng trước một ngày nhất định không?',
        titlebody:
          'Xin lưu ý rằng chúng tôi phải xử lý đơn đặt hàng của bạn trước khi nó được vận chuyển. Điều này có thể mất  1-3 ngày làm việc. Sau khi các mặt hàng của bạn được xử lý, chúng sẽ vận chuyển đơn hàng nhanh cho bạn. Mặc dù chúng tôi cố gắng giao hàng trong thời gian nhanh nhất, nhưng chúng tôi không thể đảm bảo ngày giao hàng chính xác. Cảm ơn vì sự kiên nhẫn của bạn. Để xem thêm thông tin, vui lòng kiểm tra:\n"Thông Tin Vận Chuyển"\nGiao Hàng tiêu chuẩn\nThời gian:\nGiao hàng dự kiến vào ngày 31/08/2022 - 02/09/2022.\nChi phí vận chuyển:\nToàn bộ miễn phí vận chuyển',
        checkItem: false,
      },
      {
        id: 2,
        title: 'Nếu đã xác nhận giao hàng trước khi giao hàng thì phải sao?',
        titlebody:
          'Nó sẽ không ảnh hưởng đến việc vận chuyển ngay cả khi bạn nhấp vào " xác nhận giao hàng "trước khi giao hàng. Bạn có thể tìm thấy số theo dõi và liên kết theo dõi trong email xác nhận vận chuyển để theo dõi đơn đặt hàng của mình hoặc bạn có thể nhấp vào nút ở góc trên bên phải liên hệ với chúng tôi.',
        checkItem: false,
      },
      {
        id: 3,
        title: 'Tại sao đơn hàng của tôi bị trì hoãn?',
        titlebody:
          'Chào bạn, chúng tôi xin lỗi vì sự chậm trễ.\n' +
          'Trước hết,thông thường chúng tôi phải mất 1-3 ngày để chuẩn bị các' +
          ' mặt hàng của bạn. Và kể từ khi chúng tôi chuyển ra nước ngoài, sẽ mất vài ngày để thông quan.' +
          ' Trong thời gian này, nó có lẽ không có cập nhật cho vận chuyển.\n' +
          'Do ảnh hưởng của dịch COVID-19, việc thiếu tài nguyên vận tải toàn' +
          ' cầu có lẽ làm trì hoãn đơn hàng của bạn trong vài ngày.\n' +
          'Bạn có thể tiếp tục chờ đợi thêm một vài ngày nữa không?\n' +
          'Bạn cũng có thể liên hệ với chúng tôi để biết thêm thông tin theo dõi nếu cần. ' +
          'Cảm ơn cho sự hiểu biết và hợp tác của bạn.',
        checkItem: false,
      },
      {
        id: 4,
        title: 'Tại sao mặt hàng của tôi bị mất?',
        titlebody:
          'Xin chào, xin lỗi về sự bất tiện này,vui lòng nhấp vào nút ở góc trên bên phải liên hệ với chúng tôi  về vấn đề này. Vui lòng cung cấp cho chúng tôi hình ảnh của toàn bộ gói bên ngoài bao gồm hai nhãn vận chuyển,' +
          ' cũng như mặt hàng nào bị thiếu, để dịch vụ khách hàng của chúng tôi có thể giúp bạn tốt hơn.',
        checkItem: false,
      },
      {
        id: 7,
        title:
          'Tôi nhận thấy rằng gói của tôi đã bị hải quan từ chối. Tôi có thể làm gì?',
        titlebody:
          'Xin chào. Chúng tôi rất xin lỗi về điều này. Họ đã cung cấp cho bạn những thông tin nào? Vui lòng liên hệ với dịch vụ khách hàng của chúng tôi và cung cấp cho chúng tôi thông tin đó. Vì vậy, chúng tôi có thể giúp bạn với một cách tốt hơn.',
        checkItem: false,
      },
      {
        id: 10,
        title: 'Mất bao lâu để gói hàng đến đích?',
        titlebody:
          'Chào bạn, thông thường sẽ mất 1-3 ngày để chuẩn bị đồ. Thời gian giao hàng là khoảng 8-11 ngày. Nó có thể thay đổi do địa chỉ không hợp lệ, thủ tục thông quan hoặc các nguyên nhân khác. Thời gian giao hàng thực tế tùy thuộc vào trang web chính thức hoặc thông báo của công ty hậu cần.\n' +
          'Để biết thêm chi tiết, vui lòng nhấp vào trang này: "Thông Tin Vận Chuyển"\n' +
          'Lưu ý: Thời gian giao hàng = Thời gian xử lý + Thời gian vận chuyển\n' +
          'Do ảnh hưởng của dịch COVID-19, việc thiếu nguồn lực vận chuyển toàn cầu có thể khiến đơn đặt hàng của bạn bị trì hoãn trong vài ngày.\n' +
          'Cảm ơn sự hiểu biết của bạn.',
        checkItem: false,
      },
      {
        id: 11,
        title: 'Đơn hàng của tôi ở đâu?',
        titlebody:
          'Nếu không có thông tin cập nhật tại theo dõi trang sau khi đơn hàng của bạn được chuyển đi thì nguyên nhân là đơn hàng của bạn được gửi từ các kho hàng ở nước ngoài của chúng tôi. Các bưu kiện cần phải đến Việt Nam từ nước ngoài.\n' +
          'Sau đó công ty vận chuyển địa phương sẽ nhận bưu kiện từ cảng trong ngày làm việc. Sau khi công ty vận chuyển địa phương quét các bưu kiện đó, trang web sẽ cập nhật số theo dõi.\n' +
          'Vui lòng dành thời gian để nhân viên chuyển phát cập nhật thông tin theo dõi mới nhất sau khi đến trạm tiếp theo, hy vọng bạn sẽ sớm nhận được gói hàng và yêu thích món đồ tuyệt vời.\n',
        checkItem: false,
      },
    ],
    check: false,
  },
  {
    id: '3',
    headerTitle: 'Trả hàng và Hoàn tiền',
    nameIcon: 'archive',
    data: [
      {
        id: 1,
        title: 'Số tiền hoàn trả của tôi không chính xác?',
        titlebody:
          'Xin chào, việc hoàn tiền sẽ chỉ dành cho các mặt hàng, những phí khác như bảo hiểm vận chuyển và phí vận chuyển sẽ không được hoàn trả.',
        checkItem: false,
      },
      {
        id: 2,
        title: 'Nếu tôi nhận được một mục sai thì sao?',
        titlebody:
          'Xin chào,Cảm ơn bạn đã đặt hàng từ trang web của chúng tôi. Chúng tôi xin lỗi rằng các mặt hàng của chúng tôi không đáp ứng được sự mong đợi của bạn.Bạn có thể vui lòng liên hệ với chúng tôi để kết nối với chúng tôi bằng hình ảnh của hàng đã nhận mà gặp vấn đề này không? Sau đó, chúng tôi có thể cung cấp cho bạn giải pháp tốt nhất để đáp ứng sự hài lòng của bạn.Hy vọng điều này sẽ giúp bạn.',
        checkItem: false,
      },
      {
        id: 3,
        title: 'Tôi có thể được hoàn tiền đầy đủ cho việc trả lại không?',
        titlebody:
          'Xin chào, Chúng tôi sẽ hoàn lại tiền đầy đủ cho tất cả các mặt hàng bạn đã trả lại, không bao gồm phí vận chuyển trả lại và các mặt hàng không thể hoàn trả. Mã giảm giá và điểm Fashion sẽ không được hoàn trả. Hy vọng điều này sẽ giúp được bạn.',
        checkItem: false,
      },
      {
        id: 4,
        title: 'Tôi có thể trả lại hàng của mình qua bưu điện nào?',
        titlebody:
          'Xin chào, chúng tôi không có giới hạn cho việc này. Bạn có thể chọn các công ty vận chuyển sử dụng nhiều nhất, chẳng hạn như bưu điện địa phương để trả lại hàng cho chúng tôi. Và phí vận chuyển của bưu điện thấp hơn các công ty vận chuyển khác.',
        checkItem: false,
      },
      {
        id: 5,
        title: 'Chính sách hoàn trả của bạn là gì?',
        titlebody:
          'Chúng tôi có thể chấp nhận trả lại hầu hết các mặt hàng trong vòng 15 ngày kể từ khi nhận được trong tình trạng mới. Cũng xin lưu ý rằng các mặt hàng sau không được phép trả lại hoặc trao đổi: áo liền quần, nội y, đồ bơi,vật tư sự kiện &amp; tiệc, đồ DIY, đồ dùng thú cưng, đồ trang sức, đồ làm đẹp và phụ kiện (trừ khăn quàng cổ, túi xách,và chăn nàng tiên cá).',
        checkItem: false,
      },
      
    ],
    check: false,
  },
  {
    id: '5',
    headerTitle: 'Sản phẩm và Kho hàng',
    nameIcon: 'shirt',
    data: [
      {
        id: 1,
        title: 'Làm thế nào để tôi chọn kích thước của tôi?',
        titlebody:
          'Hãy lấy số đo của bạn và so sánh với Biểu đồ kích thước chi tiết của từng sản phẩm.Bạn có thể tìm thấy một tham chiếu kích thước ở bên phải của ảnh sản phẩm trên trang sản phẩm.Điều này sẽ giúp bạn chọn kích thước nào là phù hợp nhất với cơ thể của bạn. Một kích thước không nhất thiết phải phù hợp với tất cả các kích cỡ nhưng có nghĩa là chỉ có một kích thước cho sản phẩm này.Ngoài ra, rất vui khi giới thiệu chức năng lọc kích thước Fashion cho bạn, nó được gọi là " Kiểm tra kích thước của tôi ". Điền vào kích thước của riêng bạn ở đó để có được một kích thước cụ thể được đề nghị cho bạn.',
        checkItem: false,
      },
      {
        id: 2,
        title: 'Bạn sẽ bổ sung các mặt hàng đã bán hết không?',
        titlebody:
          'Xin chào. Chúng tôi xin lỗi vì sự bất tiện này. Nếu một mục được đánh dấu là "Đã bán hết" hoặc không có kích thước bạn muốn, chúng tôi đề nghị bạn nên kiểm tra trang web thường xuyên trong vòng hai tuần tới trong trường hợp mục đó được khôi phục. Tuy nhiên, nếu nó không được khôi phục trong vòng vài tuần tới, nó có thể sẽ bị xóa khỏi cửa hàng trực tuyến của chúng tôi.Bạn có thể chọn mua các sản phẩm tương tự khác.',
        checkItem: false,
      },
      
    ],
    check: false,
  },
  {
    id: '6',
    headerTitle: 'Tài khoản',
    nameIcon: 'person',
    data: [
      {
        id: 1,
        title: 'Tại sao chúng tôi đăng ký cho bạn một tài khoản trực tuyến?',
        titlebody:
          'Xin chào, nếu bạn muốn bắt đầu mua hàng, trước tiên bạn cần đăng ký tài khoản Fashion.Sau khi bạn đăng ký tài khoản Fashion, bạn có thể tận hưởng dịch vụ tốt hơn từ chúng tôi, ngoài ra chúng tôi sẽ cập nhật mọi thứ về giao dịch mua của bạn trong tài khoản Fashion của bạn. Sau đó, bạn có thể theo dõi đơn đặt hàng, kiểm tra tiền hoàn lại và số điểm, v.v. trong tài khoản Fashion của mình.Ngoài ra, xin đừng lo lắng, chúng tôi sẽ không tính phí bạn bất cứ điều gì về tài khoản này.',
        checkItem: false,
      },
      {
        id: 2,
        title: 'Tại sao tôi không thể đăng nhập vào tài khoản của mình?',
        titlebody:
          'Xin chào, cho vấn đề này:Hãy đảm bảo rằng bạn đang sử dụng đúng địa chỉ email và mật khẩu mà bạn đã đăng ký. Nếu bạn quên mật khẩu, bạn có thể nhấp vào nút “Quên mật khẩu” khi đăng nhập, sau đó bạn có thể đặt lại mật khẩu mới.Vui lòng lưu ý rằng bạn có thể có 2-3 tài khoản Fashion có cùng địa chỉ email, chẳng hạn như tài khoản thông thường bạn đăng ký bằng địa chỉ email, tài khoản Facebook và tài khoản Google của bạn. Hãy chắc chắn rằng bạn đã sử dụng để đăng nhập.Ngoài ra nếu địa chỉ email bị sai, bạn có thể liên hệ với chúng tôi để được trợ giúp.',
        checkItem: false,
      },
      {
        id: 3,
        title: 'Làm cách nào để đặt lại mật khẩu?',
        titlebody:
          'Bạn vui lòng làm theo các bước dưới đây:1. Nhấp vào "Đăng nhập" ở góc trên cùng bên phải của trang chủ của chúng tôi.2. Chọn "Quên mật khẩu"3. Nhập địa chỉ email đã đăng ký của bạn vào ô được cung cấp.4. Bạn sẽ nhận được email đặt lại và bạn có thể tạo một mật khẩu mới qua liên kết. Cũng vui lòng kiểm tra thư rác của bạn nếu bạn không thể tìm thấy Email của chúng tôi.',
        checkItem: false,
      },
      {
        id: 4,
        title: 'Làm cách nào để thay đổi chi tiết tài khoản của tôi?',
        titlebody:
          'Xin chào, bạn có thể thông qua cài đặt tài khoản của tôi "để thay đổi chi tiết tài khoản " . Nhưng chúng tôi rất tiếc phải thông báo cho bạn rằng: Khách hàng không thể cập nhật địa chỉ email. Vui lòng sử dụng địa chỉ email đã đăng ký ban đầu để đăng nhập vào trang web của chúng tôi. Nếu bạn Không có bất kỳ đơn đặt hàng nào đang chờ xử lý trong tài khoản này, đề nghị bạn có thể liên hệ với dịch vụ khách hàng của chúng tôi bằng cách nhấp vào nút ở góc trên bên phải liên hệ với chúng tôi  để xóa nó, sau đó bạn có thể đăng ký lại với email chính xác.',
        checkItem: false,
      },
      {
        id: 5,
        title: 'Làm thế nào để đăng ký?',
        titlebody:
          'Cảm ơn bạn đã quan tâm đến trang web của chúng tôi, đây là các bước: 1. Cuộn xuống cuối trang chủ của chúng tôi 2. Điền địa chỉ email của bạn 3. Nhấp vào đăng ký để gửi.',
        checkItem: false,
      },
     
    ],
    check: false,
  },
];
